...
class Game():
    ...
    """
    def do_take_card(self) -> Card:
        prompt = 'Draw from (d)eck'
        if self.throw_card is not None:
            prompt += ' or (t)hrow card'
        prompt += ': '
        c = None
        while True:
            c = input(prompt)
            if c == 't' and self.throw_card is not None:
                return self.throw_card
            elif c == 'd':
                # throw card gone forever:
                self.deck.discard(self.throw_card)
                return self.deck.draw()

    def do_discard(self) -> Card:
        while True:
            c = input('Discard: ')
            try:
                return self.cur_player.discard(int(c))
            except ValueError:
                continue
            except IndexError:
                continue
    ...
    def do_round(self) -> None:
        new_card = self.do_take_card()
        print('New card: {}'.format(new_card))
        self.cur_player.add_card(new_card)
        self.cur_player.show_hand()
        self.throw_card = self.do_discard()
        print('Discarded: {}'.format(self.throw_card))
    """

"""
if __name__ == '__main__':
    #c = Card(Suit.HEARTS, Rank.TEN)
    #c2 = Card(Suit.DIAMONDS, Rank.TWO)
    #print(c, c2, c+c2)
    d = Deck()
    p1 = Player()
    g = Game(d, [p1])
